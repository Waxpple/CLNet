...
class Trainer:
    ...
    def __init__(self, model, device, optimizer, criterion, scheduler, chk_name, neptune_run, resume=None,
        ...
        self.tester = Tester(model, device, criterion, neptune_run, print_freq)
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, ) in enumerate(data_loader):
            ...
            self.neptune_run["train_batch_idx"].log(batch_idx + 1)
            self.neptune_run["train_epoch"].log(self.cur_epoch)
            self.neptune_run["train_loss"].log(iter_loss.avg)
            self.neptune_run["train_lr"].log(self.scheduler.get_lr()[0])
            self.neptune_run["train_time"].log(iter_time.avg)
...
class Tester:
    ...
    def __init__(self, model, device, criterion, neptune_run, print_freq=20):
        self.model = model
        self.device = device
        self.criterion = criterion
        self.print_freq = print_freq
    ...
    def _iteration(self, data_loader):
        ...
        for batch_idx, (sparse_gt, raw_gt) in enumerate(data_loader):
            ...
            self.neptune_run["test_batch_idx"].log(batch_idx + 1)
            self.neptune_run["test_loss"].log(iter_loss.avg)
            self.neptune_run["test_rho"].log(iter_rho.avg)
            self.neptune_run["test_NMSE"].log(iter_nmse.avg)
            self.neptune_run["test_time"].log(iter_time.avg)
