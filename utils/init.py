...
def init_model(args):
    ...
    elif args.model == 'culnet':
        model = culnet(reduction=args.cr)
    ...
    flops, params = thop.profile(model.encoder, inputs=(image,), verbose=False)
    flops, params = thop.clever_format([flops, params], "%.3f")
    logger.info(f'=> Model encoder Flops: {flops}')
    logger.info(f'=> Model encoder Params Num: {params}\n')
    codeword = torch.randn([1,2048//args.cr])
    flops, params = thop.profile(model.decoder, inputs=(codeword,), verbose=False)
    flops, params = thop.clever_format([flops, params], "%.3f")
    logger.info(f'=> Model decoder Flops: {flops}')
    logger.info(f'=> Model decoder Params Num: {params}\n')
    return model
