...
class CRBlock(nn.Module):
    def __init__(self):
        ...
        self.path1 = nn.Sequential(OrderedDict([
            ...
            ('conv1x9', ConvBN(7, 7, [1, 3])),
            ('relu2', nn.LeakyReLU(negative_slope=0.3, inplace=True)),
            ('conv9x1', ConvBN(7, 7, [3, 1])),
        ]))
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        encode2_1 = self.sa(encode2)
        #print(encode2_1.shape)
        #out = torch.cat((encode1, encode2), dim=1)
        #out = self.encoder_conv(out)
        encode2_2 = self.se(encode2_1)
        out = self.conv3(encode2_2)
        
        out = out.view(n, -1)
        #out = out.unsqueeze(2) #[1,2048,1]
        out = self.encoder_fc(out)
#         m1 = self.encoder_fc_comp_1(out)
#         a1 = self.encoder_fc_comp_2(out)
#         out = self.encoder_fc_comp_3(torch.cat((m1,a1), dim=1))
